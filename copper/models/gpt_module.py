...
class GPT(nn.Module):
    def __init__(self, vocab_size, block_size, n_embed, n_heads, drop_p, n_decoder_blocks):
        ...
        block_list = [GPTDecoderBlock(emb_size=n_embed, n_heads=self.n_heads, drop_p=self.drop_p) for i in range(0,self.n_decoder_blocks)]
        #self.blocks = nn.ModuleList([
        #    GPTDecoderBlock(emb_size=n_embed, n_heads=self.n_heads, drop_p=self.drop_p),
        #    GPTDecoderBlock(emb_size=n_embed, n_heads=self.n_heads, drop_p=self.drop_p),
        #    GPTDecoderBlock(emb_size=n_embed, n_heads=self.n_heads, drop_p=self.drop_p),
        #    GPTDecoderBlock(emb_size=n_embed, n_heads=self.n_heads, drop_p=self.drop_p),
        #])
...
# Lightning model for GPT architecture
